...
@tembm.on(events.NewMessage(outgoing=False, pattern='/bm1'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await tembm.send_message(event.chat_id, f"تم تجميع نقاط...! ")
        ...
        await tembm.send_message(event.chat_id, "تم تجميع نقاط...! ")
